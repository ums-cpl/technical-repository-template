...
**Case file format:** Case files use the `.expected` suffix (e.g. `build.expected`). Comment lines (starting with `#` after optional leading whitespace) are ignored throughout. After skipping comments: the first line is the invocation args for `run_tasks.sh` (e.g. `tasks/build`), the second must be exactly `---`, and the rest is the expected manifest (same format as `--dry-run` output). You can use comments to document the test's purpose, including at the top of the file. On manifest diff failure, the actual output is saved to a file with the same base name and `.actual` suffix (e.g. `build.actual`).